...
def print_exp(tree: BinaryTree) -> str:
    s_val = ""
    if tree:
        if print_exp(tree.get_left_child()).isdecimal() and print_exp(tree.get_right_child()).isdecimal():
            s_val = '(' + print_exp(tree.get_left_child())
            s_val = s_val + str(tree.get_root_val())
            s_val = s_val + print_exp(tree.get_right_child()) + ')'
        if print_exp(tree.get_left_child()).isdecimal():
            s_val = '(' + print_exp(tree.get_left_child())
            s_val = s_val + str(tree.get_root_val())
            s_val = s_val + print_exp(tree.get_right_child())
        elif print_exp(tree.get_right_child()).isdecimal():
            s_val = print_exp(tree.get_left_child())
            s_val = s_val + str(tree.get_root_val())
            s_val = s_val + print_exp(tree.get_right_child()) + ')'
        else:
